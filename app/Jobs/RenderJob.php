...
class RenderJob implements ShouldQueue, ShouldBeUnique
{
    ...
    public function handle()
    {
        ...
        if ($this->assetType == 'clothing') {
            $this->assetJob['3d'] = true;
            $clothing = file_get_contents('http://127.0.0.1:64989/render/clothing/'.$this->assetId.'?key='.$arbiterkey);
            $image = new SimpleImage();
            $image = $image
                ->fromString($this->decode($clothing))
                ->resize($x, $y)
                ->toString();
            Render::save(sprintf('%s/%d.png', $this->assetJob['folder'], $this->assetId), $image);
        }
        if ($this->assetType == 'place' || $this->assetType == 'serverplace') {
            $place = file_get_contents('http://127.0.0.1:64989/render/game/'.$this->assetId.'?key='.$arbiterkey);
            $image = new SimpleImage();
            $image = $image
                ->fromString($this->decode($place))
                ->resize($x, $y)
                ->toString();
            Render::save(sprintf('%s/%d.png', $this->assetJob['folder'], $this->assetId), $image);
        }
        try {
            $type = "";
            if ($this->assetJob['folder'] != "users" && $this->assetType != 'clothing' && $this->assetType != 'place' && $this->assetType != 'serverplace') {
        ...
        if ($this->assetJob['3d'] || $this->assetType == 'clothing')  // THE LAZY WAY OUT
        {
            $job_id = (string) Str::uuid();
            ...
            try
            {
                ...
                } elseif ($this->assetType == 'clothing') {
                    $result = file_get_contents('http://127.0.0.1:64989/render/clothing/'.$this->assetId.'/3d?key='.$arbiterkey);
                } else {
